...
const SermonCard = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
      <div className="relative bg-[#EDEDED] text-[#DA0037] p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-2xl font-serif font-bold">{title}</h3>
          <Video className="text-[#DA0037] opacity-50" size={32} />
        </div>
        <div className="flex items-center justify-between">
          <div>
            ...
            {series && (
              <p className="text-sm text-[#DA0037] mt-1">Series: {series}</p>
            )}
          </div>
          {duration && (
        ...
        {/* Display Date */}
        <p className="text-sm text-[#DA0037] mt-2">{date}</p>
      </div>
      <div className="p-6">
        {description && (
        ...
        <div className="flex space-x-4">
          <a
            ...
            className="flex-grow flex items-center justify-center bg-red-50 text-[#DA0037] py-3 rounded-lg hover:bg-red-100 transition"
          >
            <PlayCircle className="mr-2" size={20} />
            Watch Sermon
          </a>
          <button className="bg-gray-50 text-gray-600 px-4 py-3 rounded-lg hover:bg-gray-100 transition">
            <Download size={20} />
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-row justify-center align-middle gap-4 px-4 mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
          {categories.map((category) => (
            <button
              ...
              className={`
                flex items-center px-2 py-1 rounded-md transition 
                ${
      ...
      <div className="bg-white py-16 shadow-lg rounded-lg">
        <div className="container max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="flex justify-center space-x-4">
            <a
              href="https://podcasts.apple.com/" // Replace with your Apple Podcasts link
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center bg-[#7E1891] text-white px-8 py-3 rounded-lg hover:bg-[#7E5CAD] transition shadow-md hover:shadow-lg"
            >
              <FaApple className="mr-2" size={20} />
              Apple Podcasts
            </a>
            <a
              href="https://open.spotify.com/" // Replace with your Spotify link
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center bg-green-600 text-white px-8 py-3 rounded-lg hover:bg-green-700 transition shadow-md hover:shadow-lg"
            >
